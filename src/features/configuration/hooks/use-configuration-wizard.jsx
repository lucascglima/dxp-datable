 ...
 * Note: 'json' step is special - it's used for JSON editor mode but doesn't appear in visual steps
 */
...
/**
 * Special step key for JSON mode (not in visual step list)
 */
export const JSON_STEP_KEY = 'json';
...
export const useConfigurationWizard = () => {
   ...
   * JSON mode returns -1 (not a visual step)
   */
  const currentStep = useMemo(() => {
    const stepKey = urlNav.step;
    if (!stepKey) return 0; // Default to first step
    if (stepKey === JSON_STEP_KEY) return -1; // JSON mode (not in visual steps)
  ...
  /**
   * Checks if current mode is JSON
   */
  const isJsonMode = urlNav.step === JSON_STEP_KEY;
